...
function updateCards(planets, skip) {
  ...
  for(i=0;i<curPlanets.length;i++) { //stuff should say 'N/A' unless there is data to fill it.
    ...
    var card = ["<div class=\"content planetsCard\">",
      ...
      "<h5>"+($(curPlanets[i]).find("description").text()!==""?$(curPlanets[i]).find("description").text() : "This planet has no description.")+"</h5>",
      ...
      "<h4>Star's Mass:</h4>",
      "<p class=\"detailText\" id=\"value5\">"+($(curPlanets[i]).find("mass").text()!==""?$(curPlanets[i]).find("mass").text() : "N/A")+"</p>",
      "<h4><span class=\"hoverInfoTemp\">Star's Temperature (K)<sup>?</sup></span>:</h4>",
      "<p class=\"detailText\" id=\"value6\">"+($(curPlanets[i]).find("temperature").text()!==""?$(curPlanets[i]).find("temperature").text() : "N/A")+"</p>",
      "</div>",
      "</div>"].join('\n');
